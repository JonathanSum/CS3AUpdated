...
********************************************/
    Date(unsigned m,
        ...
        //PROCESSING:  Debug first, if month is greater than 12 or less than 1, day is less than one or greater
        // than 31, year is less than 0, or day is greater than 28 in Feb
        if (m > 12 || m < 1 || d > 31 || d < 1 || y < 0 || (m == 2 && d > 28)) {
            ...
            if (m < 1) {
                string line = "Invalid date values: Date corrected to 1/" +to_string(day)+ "/"+to_string(y )+".";
                QMessageBox msgBox;
                msgBox.setText( QString::fromStdString(line));
                msgBox.exec();
                cout << "Invalid date values: Date corrected to 1/" << day << "/" << y << "." << endl;
                //OUTPUT        correcting the error input of the date class
                month = 1;
                monthName = "January";
                m = 1;

            } else if (m > 31) {
                string line = "Invalid date values: Date corrected to 12/" +to_string(day)+ "/"+to_string(y )+".";
            ...
            //debug invalid day
            if (d < 1) {
                string line = "Invalid date values: Date corrected to " +to_string(month)+ "/1/" +to_string( y) + ".";
                QMessageBox msgBox;
                msgBox.setText( QString::fromStdString(line));
                msgBox.exec();
            ...
            if (d > 31 && m != 2) {
                ...
                string line = "Invalid date values: Date corrected to " +to_string(month)+ "/"
                        +to_string(day)+ "/" +to_string( y) + ".";
                QMessageBox msgBox;
                msgBox.setText( QString::fromStdString(line));
                msgBox.exec();
                ...
                monthName = "February";
                day = 28;
                string line = "Invalid date values: Date corrected to "
                        +to_string(month)+ "/"+to_string(day)+ "/" +to_string( y) + ".";
                QMessageBox msgBox;
                msgBox.setText( QString::fromStdString(line));
                msgBox.exec();
                cout << "Invalid date values: Date corrected to " << month << "/" << day << "/" << y << "." << endl;
            ...
            //OUTPUT:   set the month, day, and year to the private section
            // of the class if they are passed the debug section.
...
********************************************/
    Date(const string &mn,
        ...
        int m =MonthNumReturn(mn);
        //OUT  It will return to 0 if the name is not in the month list in English,
        // else it will convert month name in string to number of the month.
...
//PROCESSING if incorrected date values were input
        if ( d > 31 || d < 1 || y < 0 || (m == 2 && d > 28)) {
            ...
            if (d > 31 && m != 2) {
                ...
                string line = "Invalid date values: Date corrected to "
                        +to_string(month)+  "/" +"31"+"/"+to_string( y) + ".";
                QMessageBox msgBox;
                msgBox.setText( QString::fromStdString(line));
                msgBox.exec();
                cout << "Invalid date values: Date corrected to " << month
